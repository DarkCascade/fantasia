...
ITERATIONS = [
    ...
    dict(v="v3", title="Retuning the delvers", changes=[
        ...
        ("Hulda: a high {C} (7–10, or the Jack, which matches as a 10) played as Pilfer takes 2 glimmer instead of 1.",
        ...
        ("Old Quill: his Jack Shortcut moves 4 chambers instead of 3.",
         lambda P: f"Old Quill was {pp(P.delta(2, 'quill'))} / {pp(P.delta(3, 'quill'))} / {pp(P.delta(4, 'quill'))}. "
                   "Win rates are shared, so raising three delvers pulls the other three down; Quill was already "
                   "below par with 2 players and would have sunk further, so he got a small boost too."),
        ("No change to Mira or Sable.",
         lambda P: f"They led: Mira {pp(P.delta(2, 'mira'))} / {pp(P.delta(3, 'mira'))} / {pp(P.delta(4, 'mira'))}, "
                   f"Sable {pp(P.delta(2, 'sable'))} / {pp(P.delta(3, 'sable'))} / {pp(P.delta(4, 'sable'))}. Rather "
                   "than weaken abilities that play well, the trailing delvers were brought up to meet them."),
        ("The first player's short first turn now applies only with 3 or 4 players.",
         lambda P: f"The first player won {pc(P.seat1(2), 1)} / {pc(P.seat1(3), 1)} / {pc(P.seat1(4), 1)} with "
                   ...
                   + ("With 2 players the one-action first turn over-corrected; with 3 and 4 it was spot on."
                      if P.seat1(2) < 0.5 else "")),
    ]),
...
def summary_bullets(r: Res) -> list[str]:
    ...
    if r.has("strategies"):
        out.append(f"Planning AI wins {pc(r.strat('2p:tactician/greedy', 'tactician'))} against greedy and "
                   f"{pc(r.strat('2p:tactician/cautious', 'tactician'))} against cautious; greedy wins "
                   f"{pc(r.strat('2p:greedy/cautious', 'greedy'))} against cautious (2 players).")
...
def write_changelog(res: dict[str, Res], path: Path):
    L = ["# Glimmerdark balance changelog", "",
         ...
         f"plus the strategy, luck, playtime and ability-duel runs at the same count. Playtime assumes {SEC_PER_TURN} s per player turn "
...
def story(res):
    ...
    s.append(P("Why the collapse clock works: the marker moves on every reshuffle and every joker, so a game lasts "
               "about the same number of trips through the deck however the players behave. That is why no simulated game "
    ...
    h2 = a.hist(2)
    tot2 = sum(h2.values())
    short2 = sum(v for k, v in h2.items() if int(k) < 30) / tot2
    long2 = sum(v for k, v in h2.items() if int(k) > 45) / tot2
    s.append(P(f"The 2-player games that miss the window mostly miss it short: {pc(short2)} finish under 30 minutes "
               f"and {pc(long2)} run past 45 (at {SEC_PER_TURN} s a turn). A slower table lands them inside it.",
               "small"))
...
def RANK_USES(by_rank):
    """Dead cards would show up as ranks nobody plays, or ranks with only one use."""
    total = sum(by_rank[k]["plays"] for k in RANKS)
    share = min(by_rank[k]["plays"] / total for k in RANKS) * len(RANKS)
    two = [k for k in RANKS if sum(1 for a, v in by_rank[k].items() if a != "plays" and v >= 0.10) >= 2]
    plural = {"11": "Jacks", "12": "Queens", "13": "Kings", "14": "Aces, which are wild and nearly always mine"}
    single = [plural.get(k, f"{k}s") for k in RANKS if k not in two]
    t = (f"Every rank gets played: the least-played comes up {pc(share)} as often as an even share would give. "
         f"{len(two)} of the 13 ranks are regularly played two or more different ways (each at least 10% of the "
         "time), so a card is rarely stuck with one use")
    return t + (f"; the exception: {', '.join(single)}." if single else ".")
